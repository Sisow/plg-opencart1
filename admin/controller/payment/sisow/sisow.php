...
class ControllerPaymentSisow extends Controller {
	...
	public function _index($payment) {
		...
		// Sisow payment fee tonen bij checkout
		$do = false;
		...
		if ($payment == 'sisowklarna' || $payment == 'sisowklaacc' || $payment == 'sisowfocum') {
			...
			$this->data['entry_ovbprefix'] = $this->language->get('entry_ovbprefix');
		}
		...
		{
			$this->data['entry_b2b'] = $this->language->get('entry_b2b');
		}
		
		$this->data['entry_autoinvoice'] = $this->language->get('entry_autoinvoice');
		$this->data['entry_paymentfee'] = $this->language->get('entry_paymentfee');
		$this->data['entry_'.$payment.'fee_tax'] = $this->language->get('entry_sisowfee_tax');
		$this->load->model('localisation/tax_class');
		$this->data['tax_classes'] = $this->model_localisation_tax_class->getTaxClasses();
		...
		// Auto invoice
		if (isset($this->request->post[$payment . '_autoinvoice'])) {
			$this->data[$payment . '_autoinvoice'] = $this->request->post[$payment . '_autoinvoice'];
		}
		else {
			$this->data[$payment . '_autoinvoice'] = $this->config->get($payment . '_autoinvoice');
		} 
		...
		if (isset($this->request->post[$payment . '_shopid'])) {
			$this->data[$payment . '_shopid'] = $this->request->post[$payment . '_shopid'];
		}
		else {
			$this->data[$payment . '_shopid'] = $this->config->get($payment . '_shopid'); 
		} 
		...
		if ($payment == 'sisowklarna' || $payment == 'sisowklaacc'|| $payment == 'sisowfocum') {
			...
			if ($payment == 'sisowklarna' || $payment == 'sisowklaacc') {
				...
				if (isset($this->request->post[$payment . '_status_pending'])) {
					$this->data[$payment . '_status_pending'] = $this->request->post[$payment . '_status_pending'];
				}
				else {
					$this->data[$payment . '_status_pending'] = $this->config->get($payment . '_status_pending'); 
				}
				// Klarna Merchant ID
				if (isset($this->request->post[$payment . '_klarnaid'])) {
					$this->data[$payment . '_klarnaid'] = $this->request->post[$payment . '_klarnaid'];
				}
				else {
					$this->data[$payment . '_klarnaid'] = $this->config->get($payment . '_klarnaid'); 
				}
			}
		}
		else if ($payment == 'sisowovb') {
			...
			if (isset($this->request->post[$payment . '_prefix'])) {
				$this->data[$payment . '_prefix'] = $this->request->post[$payment . '_prefix'];
			}
			else {
				$this->data[$payment . '_prefix'] = $this->config->get($payment . '_prefix');
			}
		}
		else if($payment == 'sisowafterpay')
		{
			...
			if (isset($this->request->post[$payment . '_useb2b'])) {
				$this->data[$payment . '_useb2b'] = $this->request->post[$payment . '_useb2b'];
			}
			else {
				$this->data[$payment . '_useb2b'] = $this->config->get($payment . '_useb2b');
			} 
		}
